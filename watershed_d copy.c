...
iftImage* FWatershed(iftMImage* img, iftLabeledSet* S) {
  ...
  // build an array of number of pixels associated to the seed
  // int n_labels = iftNumberOfLabels(S);
  // int* n_pixels = (int*)malloc(n_labels * sizeof(int)); // n_pixels[ R(p) ] += 1 
  // iftMImage* suma_pixels = iftCreateMImage(n_labels, 1, 1, img->m);
